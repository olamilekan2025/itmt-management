...
export default function FinanceNotificationsPage() {
  ...
  const loadNotifications = useCallback(
    async (showRefresh = false) => {
      ...
      try {
        const response = await getAdminNotifications(accessToken, {
          page: 1,
          limit: 50,
        });
  ...
  async function handleMarkAsRead(notification: AdminNotification) {
  ...
  async function handleDelete(notification: AdminNotification) {
    if (!accessToken) {
      return;
    }
    ...
    try {
      await deleteAdminNotification(notification._id, accessToken);
        ...
        <div className="relative z-10 flex flex-col gap-6 p-6 lg:flex-row lg:items-center lg:justify-between lg:p-8">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-white/70">
              Stay updated on payment activity, verifications, and finance
              system alerts.
            </p>
          </div>
          ...
          {notifications.length === 0 ? (
            <div className="flex flex-col items-center justify-center px-6 py-20 text-center">
              ...
              <p className="mt-2 max-w-sm text-sm leading-6 text-slate-500">
                You are all caught up. New payment and finance notifications
                will appear here.
              </p>
            </div>
          ) : (
